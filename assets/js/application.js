...
function manageSidebar() {
    ...
    } else if (userAccountMenu == "large")setMenuLarge();
    else if (userAccountMenu == "medium")setMenuMedium();
    else if (userAccountMenu == "thin")setMenuThin();
    ...
    $(document).on("click", "#menu-large", function () {
        ...
        $('.sidebar-nav li.current .submenu').addClass('in').height('auto');
        if (menuWidthAction) {
            menuWidthActionFn(250);
        }
        sidebarHeight();
        createSideScroll();
        saveUserMenu("large", "menu-medium");
    });
    ...
    $(document).on("click", "#menu-medium", function () {
        ...
        $('.sidebar-nav li.current .submenu').removeClass('in');
        if (menuWidthAction) {
            menuWidthActionFn(125);
        }
        sidebarHeight();
        destroySideScroll();
        saveUserMenu("medium", "menu-thin");
    });
    ...
    $(document).on("click", "#menu-thin", function () {
        ...
        if (menuWidthAction) {
            menuWidthActionFn(50);
        }
        destroySideScroll();
        saveUserMenu("thin", "menu-large");
    });
    ...
    function saveUserMenu(menu, menuNext) {
        $.ajax({
            type: "POST",
            data: {menu: menu, next: menuNext},
            url: "/accountSettingsSaveMenu"
        });
    }
}
